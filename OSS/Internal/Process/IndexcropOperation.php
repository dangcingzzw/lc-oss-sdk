...
class IndexcropOperation{
    public static function main($params) {
        $instr_name= 'indexcrop';
        $fileName=$params['body']['file'];
        $instruction=$params['body']['instruction'];
        if(!isset($instruction['type'])){
            throw new ObsException();
        }
        return $fileName.'?x-oss-process=image/'.$instr_name.','.$instruction['type'].'_'.$instruction['value'].',i_'. $instruction['saveArea']??0;
    }
}