...
class RoundedCornersOperation{
    public static function main($params) {
        $instr_name= 'rounded-corners';
        $fileName=$params['body']['file'];
        $instruction=$params['body']['instruction'];
        if(!isset($instruction['radius'])){
            throw new ObsException();
        }
        return $fileName.'?x-oss-process=image/'.$instr_name.','.'r_'.$instruction['radius'];
    }
}